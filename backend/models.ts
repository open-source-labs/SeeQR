...
const getDBNames = function (dbType: DBType): Promise<dbDetails[]> {
  return new Promise((resolve, reject) => {
    ...
    if (dbType === DBType.Postgres) {
      ...
      query = `SELECT table_schema, ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) AS 'DB Size in MB' FROM information_schema.tables WHERE table_schema NOT IN("information_schema", "performance_schema", "mysql") GROUP BY table_schema;`;
    }
